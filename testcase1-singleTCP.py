...
def WifiNet(n, m, IP, linkconfig, req, cap, mod,ts):
    ...
    time.sleep(3)
    info('*** set queues ***\n')
    call(["sudo", "bash","SingleTCPQueueConfig.sh"])
    
    time.sleep(3)
    info('*** set flow tables ***\n')
    call(["sudo", "bash","SingTCPFlowTableConfig.sh"])    
    ...
    for i in range(1,n+1):
        ...
        time.sleep(0.2)
        src.cmdPrint('iperf -c 10.0.0.1 -t 60 -i 1 &')
        time.sleep(0.2)
...
if __name__ == '__main__':
    setLogLevel( 'info' )
    WifiNet(5,3,"127.0.0.1",[2,2,1,2,2], [1,1,0.8,0.6,0.6],[3.0, 2.0, 1.5], [1.0]*5,30)
